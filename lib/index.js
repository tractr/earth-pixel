...
module.exports = class EarthPixel {
	...
	constructor(width, type = 'meters') {
		...
		if (type === 'meters') {
            _width = EarthPixel._roundFloat(360 * (_width / EARTH_PERIMETER));
		}
    ...
    static extract(key) {
        ...
        // Guess width
        const width = EarthPixel._roundFloat(180 / _divisions);
        
        // Guess latitude
		const latitude = EarthPixel._roundFloat(_latitude * width) - 90;
		
		// Guess longitude
		const longWidth = EarthPixel._adjustWidthForLongitude(latitude, width);
        const longitude = EarthPixel._roundFloat(_longitude * longWidth) - 180;
	...
	static _adjustWidthForLongitude(latitude, width) {
		...
		const _width = EarthPixel._roundFloat(width / _cos);
