...
module.exports = class EarthPixel {
    ...
    static extract(key) {
        ...
        const latWidth = I(width);
        
        // Guess latitude
		const latitude = (_latitude * latWidth) - I(90) + (latWidth / 2);
		
		// Guess longitude
		const longWidth = I(EarthPixel._adjustWidthForLongitude(F(latitude), width));
        const longitude = (_longitude * longWidth) - I(180) + (longWidth / 2);
	...
	_subroutine(location) {
		...
		latitude = Math.floor(latitude / _latWidth) * _latWidth;
		latitude = latitude + (_latWidth / 2);
		// Last possible value
		if (latitude >= I(90)) {
			latitude = I(90) - (_latWidth / 2);
		}
        ...
        longitude = Math.min(I(180), Math.max(I(-180), longitude));
